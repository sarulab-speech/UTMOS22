...
def cross_validation(cfg):
    ...
    k_cv = 3
    keys_metrics = ["CV_test_SRCC_SYS"]
    d_cv = defaultdict(lambda: 0)
...
def fit_and_test(cfg, k_cv, i_cv):
    ...
    wandblogger = WandbLogger(project="voiceMOS2022-cross-validation-ood",entity='sarulab-voicemos',settings=wandb.Settings(start_method="fork"))
    ...
    datamodule = CVDataModule(cfg=cfg, k_cv=k_cv, i_cv=i_cv, fold_target='ood')
    ...
    if debug:
        ...
        result = trainer.logged_metrics["test_SRCC_SYS_ood_i_cv_{}_set_name_{}".format(i_cv, "fold")]
